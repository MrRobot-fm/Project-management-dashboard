...
import { useMemo, useState, type ComponentProps, type KeyboardEvent, type MouseEvent } from "react";
...
export const TaskFilter = ({
  ...
  const resetFiltersAndCloseMenu = (
    e: MouseEvent<HTMLDivElement, globalThis.MouseEvent> | KeyboardEvent<HTMLDivElement>,
  ) => {
    e.preventDefault();
    setStatus([]);
    setPriority([]);
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
          ...
          {(size === "default" && activeCount === 0) || size === "sm" ? (
            ...
            >
              ...
              <div
                ...
                onKeyDown={(e) => e.key === "Enter" && resetFiltersAndCloseMenu(e)}
      ...
      <PopoverContent align={contentAlign} className="w-[220px] p-0 text-neutral-600">
        <Command>
              ...
              <CommandGroup key={group.label} heading={group.label}>
                {group.items.map((item) => {
                  ...
                  return (
                    <CommandItem
                      key={item.value}
                      value={group.label === "Members" ? item.label : item.value}
                      onSelect={() => handleSelect(group.label as GroupLabel, item.value)}
                      disabled={
                        group.label === "Status"
                          ? !availableStatuses.includes(item.value as TaskStatus)
                          : group.label === "Priority"
                            ? !availablePriorities.includes(item.value as TaskPriority)
