...
import { UserItem } from "@/domains/shared/components/AddTeamMemberDialog";
import { AssetThumb } from "@/domains/shared/components/AssetThumb";
...
import { DatePicker } from "@/domains/shared/components/DatePicker";
...
export const EditTaskDialogContent = ({
  ...
  const [editModeActionType, setEditModeActionType] = useState<
    "edit" | "delete"
  >("edit");
  ...
  const [formState, formAction, formPending] = useActionState<
    FormState<TaskActionFn>,
    FormData
  >(
    async (_prev, formData) => {
      const response = await updateTaskAction({
        formData,
        actionType: isCreateMode
          ? "create"
          : isEditMode
            ? editModeActionType
            : "delete",
  ...
  return (
    <form
      action={formAction}
      className={cn("flex flex-col gap-1.5", isReadMode && "pb-10")}
    >
        ...
        <div className="flex flex-col gap-3 min-w-[120px] pb-5">
          ...
          <DatePicker
            ...
            triggerProps={{
              disabled: isReadMode,
              className:
                "disabled:opacity-100 disabled:[&_svg]:hidden [&_svg]:opacity-50"
            }}
          />
        </div>
      ...
      <div
        className={cn("flex flex-col gap-4 pb-5", fieldErrors?.title && "pb-0")}
      >
      ...
      {taskDetails.startDate && (
        <input
          type="hidden"
          name="startDate"
          value={taskDetails.startDate.toISOString()}
        />
      )}
      {taskDetails.dueDate && (
        <input
          type="hidden"
          name="dueDate"
          value={taskDetails.dueDate.toISOString()}
        />
      )}
      ...
      {!isReadMode && (
        <div className="ml-auto mt-4">
          <Button
            ...
            variant="outline"
            disabled={
              formPending || (isCreateMode ? !validate.success : !hasChanges)
            }
            className={cn(
              ...
              <DropdownMenuItem asChild onSelect={e => e.preventDefault()}>
                <form action={formAction} className="!p-0 w-full">
                  ...
                  >
                    Delete task
                    {formPending && (
                      <Spinner size="xs" className="stroke-rose-500" />
                    )}
                  </Button>
                  {formState.error && (
...
export const StatusSelect = ({
  ...
  return (
    <CustomSelect
      ...
      renderItem={({ value }) => (
        <StatusBadge status={value} className="mx-px" />
      )}
    />
  );
};
