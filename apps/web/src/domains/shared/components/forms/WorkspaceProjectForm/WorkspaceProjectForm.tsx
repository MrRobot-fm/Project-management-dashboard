...
import { FieldInfo } from "@/domains/shared/components/forms/FieldInfo";
import {
  priorityBadgeData,
  statusBadgeData
...
export const WorkspaceProjectForm = <T extends Project | Workspace>({
  ...
  return (
    <form
      className="flex flex-col h-full pb-14"
      onSubmit={async e => {
        e.preventDefault();
        e.stopPropagation();
    ...
    >
      <div
        className="overflow-y-auto px-8 py-4 flex flex-col gap-4"
        style={{ scrollbarColor: "lightgray white" }}
      >
      ...
      <div className="sticky bottom-0 w-full bg-white px-10 z-50 flex justify-end">
        <form.Subscribe
